...
Recently closed (2026-03-25):
- **B155d** — Drag reorder: exercise disappears then page reloads with original order. Two bugs: (1) useEffect resets `localOrder` after every parent re-render because `serverInstances` changes reference — fix: `justReorderedRef` skips the reset after user drag. (2) `handleDragEnd` fires twice due to DndContext remount during parent re-render — fix: `dragInProgressRef` guards against double-fire. Also removed `flushSync` from B155b (no longer needed with ref guards) and cleaned up all B155c diagnostic logs.
- **B155b** — (superseded by B155d)
