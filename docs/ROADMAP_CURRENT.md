...
> Last updated: 2026-07-13 (C256 — Bechtel Momentum batch 3/3: +10 drill, catalogo 242. **Bechtel Drills Manual integration CLOSED.** Nuovo finding: tie-break per ordine-file affama i drill in coda → B-VARIETY-TIEBREAK proposto.)
...
**B-VARIETY-TIEBREAK — Tie-break alfabetico su exercise_id affama gli esercizi `tech_*` (proposto da C256, meccanismo corretto in C257)** 🟡 variety bug strutturale. Meccanismo esatto (verificato in `pick_best_exercise_p0`, resolve_session.py:639-644): `base3.sort(key=(-score, exercise_id))` — a parità di punteggio (0 per quasi tutti i drill non-recenti) vince l'**ordine alfabetico dell'id**, non l'ordine del file. Tutti i drill Bechtel hanno prefisso `tech_` → coda dell'alfabeto → fuori rotazione. Simulazione sul profilo reale di Daniele (technique_focus_gym, finestra recency 3 settimane B159b): ciclo fisso di 4 sessioni, 12 drill distinti in 12 settimane; dei 19 Bechtel eleggibili appare solo `tech_foot_flyaways` (favorito dal gruppo momentum non penalizzato). **Anche post-C257 i 3 ex-orfani restano fuori rotazione** (eleggibili ma alfabeticamente ultimi). Secondo sort site: `pick_best_exercise` (~riga 1055, sort stabile su score → lì il tie-break è davvero l'ordine del file). Fix candidato: chiave finale di sort = hash deterministico di `(exercise_id, seed)` con seed = `context.target_date`/week_start (già nel resolver, righe 1483/1876) — deterministico per (utente, data), elimina il bias e ruota il pool settimana per settimana. Tocca `resolve_session.py` (P0 gate) → **Fase 1 analisi stampata nel report C257, in attesa di OK esplicito di Daniele.**
