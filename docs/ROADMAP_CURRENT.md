...
- **B151** — Settings page: loading flash + availability day order + availability save. Three bugs: (A) first load showed "—" for all fields because Clerk auth wasn't ready when `GET /api/state` fired, returning empty template. Fix: gate rendering on `useAuth().isLoaded`, re-fetch once auth ready. (B) Availability days displayed alphabetically (Fri, Mon, Sat…). Fix: iterate `WEEKDAYS` constant (Mon→Sun) instead of `Object.entries()`. (C) Removing days from availability didn't persist — deep-merge of `{}` into existing day dict was a no-op. Fix: send `null` for removed days (deep-merge assigns directly), backend normalizer treats null as rest (D150). Frontend-only.
