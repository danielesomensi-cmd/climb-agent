...
> Last updated: 2026-04-28 (B226 — Stripe webhook hardening: fail-loud (500 on handler exception → Stripe retries with backoff), in-memory LRU dedup of event.id (1024 entries, in-process), `customer.deleted` handler clears stripe IDs + sets canceled, B205 absorbed (cancel_at_period_end propagation tested). Closes audit F2+F3 + B203. Suite 1843 → 1855 (+12). Follow-up: persistent dedup table if LRU proves insufficient.)
