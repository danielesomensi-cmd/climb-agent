...
| A-TEST-REMINDER-UI | **Il promemoria di retest periodico non ha UI** | A | S | Open **P1** | Trovato da A245 G-4 (F52). Il backend è **completo**: `should_show_test_reminder()` (planner_v2) scatta ogni ~6 settimane, il payload è allegato a `GET /api/week/{n}`, esiste `POST /api/week/test-reminder-response` con confirm/postpone/skip, il tutto coperto da `test_test_week.py` e documentato in CLAUDE.md. Ma **nessun componente client lo legge**: il promemoria non ha mai raggiunto un utente. Conseguenza: la ricalibrazione periodica dei test — pilastro della metodologia 4-3-2-1 — di fatto non avviene mai, e i piani girano su baseline sempre più vecchie. Tipo `TestReminder` già aggiunto (il payload non è più invisibile); manca la card su `/today` con i 3 bottoni. **P1 e non P3** perché è metodologia, non cosmetica. |
