...
**B259 — Webhook Stripe→backend rotto (delivery mai confermata)** 🔴 **P0**, da D245. Tutti e 5 gli eventi sottoscritti (`checkout.session.completed`, `customer.subscription.updated/deleted`, `invoice.payment_succeeded/failed`) risultano **`pending_webhooks=1`** su Stripe — Stripe non ha mai ricevuto un 2xx, anche per eventi di 15 giorni fa (retry esauriti). Conseguenza: le righe `subscriptions` non vengono sincronizzate → stato desincronizzato per ogni subscription reale (caso Arnaud: Stripe `active`+pagata, DB fermo a `pending_checkout`). Endpoint `we_1TMq7RDyam3CcHNQ0Hq2Nou2` è **enabled** e punta all'URL giusto → non è config endpoint. **Ipotesi #1:** `STRIPE_WEBHOOK_SECRET` su Railway non combacia col signing secret dell'endpoint → `construct_event` solleva `SignatureVerificationError` → 400 → evento mai processato. Alternative: handler 500, env mancante. **Da fare:** verificare `STRIPE_WEBHOOK_SECRET` vs signing secret (Stripe dashboard/API), leggere Railway logs DIAG (`stripe_webhook recv … webhook_secret=…`), ritestare delivery (resend evento da Stripe), riconciliare le 2 righe drift (Arnaud `52681ef7-…` già sincronizzata a mano in D245; `98f77487-…` ancora `trialing` in DB ma `canceled` su Stripe → da riportare a `canceled`). Backend-only ma tocca billing/paywall → analisi prima. Blast radius finora basso (solo 2 sub LIVE in totale), ma critico prima di scalare gli utenti paganti. _Si interseca con B-SUB-TRIAL-RECONCILE sopra: se il webhook funzionasse, le righe `trialing` scadute verrebbero sincronizzate automaticamente._
