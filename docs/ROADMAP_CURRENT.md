...
**B259 — Webhook Stripe→backend rotto (code fix shipped, resend+reconcile pending)** 🔴 **P0**, da D245. **Root cause REALE (l'ipotesi iniziale del secret era SBAGLIATA):** `stripe.Webhook.construct_event()` ritorna uno `stripe.Event` (StripeObject) e in stripe-python ≥8 `StripeObject` **non ha `.get()`** → `event.get("id")` a `stripe_webhook.py:106` sollevava `AttributeError`, **fuori dal try/except** → **500** su ogni delivery reale → Stripe ritenta poi rinuncia (`pending_webhooks=1` per sempre) → righe `subscriptions` mai sincronizzate (caso Arnaud: Stripe `active`+pagata, DB `pending_checkout`). **Diagnosi provata via log Railway** (500 con traceback a riga 106) + test firmato col secret di Railway che **supera** la verifica firma (secret corretto). I test B226 non l'hanno preso perché mockavano `event` come `dict` (che ha `.get`). Regressione da **bump silenzioso di `stripe`** (requirements erano tutte unpinned) dopo ~16-17 apr. **Fix shipped:** (1) `event` convertito a dict una volta via `_stripe_to_dict` (`stripe_webhook.py`); (2) `stripe==15.0.0` pinnato in `backend/requirements.txt`; (3) regression test `test_b259_webhook_stripeobject.py` (8 test: StripeObject reale su tutti i rami di dispatch, asserisce 200 + payload dict). **RESIDUO (ops post-deploy, con Daniele):** dopo il deploy Railway → (a) test event da Stripe per confermare 2xx, (b) **resend** dei 5 eventi reali falliti (idempotente: non deve declassare Arnaud `active`, ripagarlo o toccare il coupon), (c) riconciliare `98f77487-…` (`trialing` in DB ma `canceled` su Stripe → `canceled`), (d) verifica immutabilità sessioni. _Si interseca con B-SUB-TRIAL-RECONCILE: con il webhook funzionante le righe `trialing` scadute si sincronizzano da sole._
