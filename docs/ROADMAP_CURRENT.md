...
Recently closed (2026-03-25):
- **B155e** — Removed exercise drag & drop reorder entirely (persistent React 18 + @dnd-kit bugs). Removed: POST /api/session/reorder-exercises endpoint, ReorderExercisesRequest model, @dnd-kit dependencies, DndContext/SortableContext/SortableExerciseItem, handleDragEnd, localOrder state, all B153-B155d reorder refs/effects. Kept: A121 phase-aware auto-ordering, exercise add/remove with _user_edited flag, trash icon for removal. −1 endpoint (50 total), −3 npm deps, −8 reorder tests.
...
Recently closed (2026-03-25):
- **B159** — Boulder surface equivalence + weight expansion. Any gym with spraywall, board_kilter, board_moonboard, or board_other now implies `gym_boulder` — sessions requiring `gym_boulder` are placeable at these gyms. Same logic for weight: dumbbell/kettlebell/barbell implies `weight`. Unified `expand_equipment()` in new `equipment_utils.py` replaces B137 homewall-specific expansion. Applied in planner, resolver, and replanner (4 code points). Fixed 5 exercises with overly-restrictive `equipment_required: ["gym_boulder"]` → `equipment_required_any` with all boulder surfaces. Fixed typo in `warmup_easy_boulders` (`kilter_board` → `board_kilter`). Daniele's Cocque gym (spraywall+kilter) now receives boulder sessions. 14 new tests (1391 total).
