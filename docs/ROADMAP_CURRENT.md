...
Recently closed (2026-03-24):
- **D152** — Add Exercise UX audit & fix for duration-based/test exercises. Phase 0 audit found 2 bugs: (1) backend `session.py` read `default_prescription` instead of `prescription_defaults` — catalog defaults never applied to manually-added exercises. (2) Add Exercise endpoint didn't propagate `category`, `attributes`, `load_model`, `unilateral` — test_measurement exercises added manually couldn't show duration input in guided mode, test results silently lost. Fix: correct field name, propagate all required fields, frontend Add Exercise dialog now shows "1 × max attempt" for test_measurement exercises and "Duration (s)" input for duration-based exercises, `formatPrescription` fallback for `sets && !reps && !workSeconds` → "N × max". 3 new backend tests (1341 total).
