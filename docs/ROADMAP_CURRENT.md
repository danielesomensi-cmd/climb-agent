...
|----|-------|----------|------|--------|--------|-------|
| — | **Session Builder** — exercise-level custom session creator | P3 | A | L | Open | User creates custom sessions picking individual exercises from the catalog (search/filter picker). Saved in `user_state.custom_sessions[]`. Treated as **free session strutturata** — no resolver, no planner placement, no automatic progression. User triggers manually from Free Session page or via "Add Session" on Week/Today. Persisted per-user in `user_state` JSONB. Entry points: Free Session page + Add Session dialog. D-type design brief required first. Origin: Christie idea (2026-04-08). |
