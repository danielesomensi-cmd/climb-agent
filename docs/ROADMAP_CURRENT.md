...
- **B155b** — Drag & drop exercise snaps back to original position on release. Root cause: React 18 batches `setLocalOrder` inside `handleDragEnd`, but @dnd-kit clears CSS transforms before React flushes the batch — causing the item to animate back to its original DOM position. Fix: wrap optimistic state updates in `flushSync()` to force synchronous DOM update before @dnd-kit cleanup.
