...
> Last updated: 2026-04-25 (D-AUDIT-RDONLY — 7-finding read-only audit. Opened: B227 (intensity_max enforcement), B226 (Stripe webhook hardening), B228 (frontend 402 global handler), D229 (doc drift), D230 (lint hygiene). Superseded: B203 → B226, B204 → B228.)
...
| GTM-STRIPE-TAX | **Stripe Tax registration** | Config | XS | 🚨 Open — urgent | Register tax ID for EU VAT. Dashboard config, not code. **Stripe LIVE since 2026-04-16 → EU VAT debt accruing on any EU customer. Do before next paid signup.** |
