...
**B-VARIETY-TIEBREAK — Tie-break per ordine-file affama gli esercizi in coda al catalogo (proposto, da C256)** 🟡 variety bug strutturale. `pick_best_exercise` ordina con sort stabile su `score_exercise`; a parità di punteggio (0 per quasi tutti i technique drill non-recenti) vince l'ordine di `exercises.json` → i drill appesi in coda (Bechtel C240/C255/C256) non entrano quasi mai in rotazione. Simulazione sul profilo reale di Daniele (technique_focus_gym, finestra recency 3 settimane B159b): ciclo fisso di 4 sessioni coi primi ~12 drill del file; dei 19 Bechtel eleggibili al suo gym ne appare **1 solo** (`tech_foot_flyaways`, favorito dal gruppo momentum non penalizzato) = 8% degli slot in 12 settimane. Fix candidato: tie-break deterministico rotation-aware (es. hash stabile di `(exercise_id, week_start)` come chiave finale di sort — resta deterministico per (utente, settimana) ma uccide il bias da posizione nel file). Tocca `resolve_session.py` (P0 gate) → richiede Fase 1 analisi + STOP. Correlato: 3 drill C256 orfani di selettore (vedi entry C256).
