...
**A245 — REVIEW-REMEDIATION-V1: bonifica consolidata frontend + backend** 🔨 P1→P3, mega-brief in 7 fasi (A→G), **una fase per sessione con OK esplicito tra una e l'altra**. Chiude 67 finding di [[D254]]. Prerequisiti soddisfatti: [[B285]] e [[B287]] entrambi in `main`. **Phase A** quick win (touch target ≥44px, tap distruttivo con conferma, timeout fetch, guard doppio tap) · **Phase B** PWA offline v1 (app shell precache + outbox generalizzato — la gap della promessa core) · **Phase C** correttezza di stato-rete (mai mappare un errore di rete su uno stato di business: un pagante offline non deve MAI vedere `/subscribe` o la welcome; +F28) · **Phase D** funnel onboarding (GTM-critical: prima di spingere traffico Reddit/IG) · **Phase E** hygiene backend (backend-only; contiene E-4 decision gate su `adaptation/closed_loop.py` dead code e E-6 su `planner_v2` → commit isolato) · **Phase F** dedup + perf leggera (+F13/F14/F15/F27) · **Phase G** correttezza data-layer + i due refactor pesanti. Fasi A/B/C/D/F/G toccano `frontend/` → branch `brief/A245-*` + preview Vercel approvata prima di ogni merge; solo E è backend-only. **Fuori scope dichiarato:** B5 (timezone del server nei path caldi — serve un A-brief proprio + D-audit prima di utenti US) e F55-F58.
...
**B-TEST-COACH-ISOLATION — I test endpoint coach non isolano il log JSONL (suite che fallisce a orologeria)** 🟡 test-only, piccolo, ma insidioso. Trovato dall'hygiene pass del 2026-07-20. I test di `/api/coach/*` girano con `user_id=None` → `storage_file._log_dir(None)` = `backend/data/logs/`, e la fixture `isolate_state` fa monkeypatch **solo di `STATE_PATH`**, non della dir di log → ogni run della suite **appende** righe a `backend/data/logs/coach_messages.jsonl` (gitignored, quindi invisibile a `git status`). Raggiunte 30 richieste utente nello stesso giorno UTC, `DAILY_MESSAGE_LIMIT` (`coach/service.py:22`) fa rispondere 429 e **3 test di `test_a243_adhoc_builder.py` falliscono** — su una code base immutata. Si auto-risolve a mezzanotte UTC, il che lo rende particolarmente ingannevole: "ho cambiato qualcosa?" no, hai solo lanciato la suite troppe volte oggi. Verificato: svuotato il file → 13/13 verdi; ogni run del file scrive 4 righe. **Fix:** monkeypatchare anche `DATA_DIR`/`USERS_DIR` (o `_log_dir`) nella fixture dei test coach verso `tmp_path`, come già si fa per lo state. Da valutare se estendere l'isolamento a tutti i test che scrivono JSONL (outdoor, session logs) — stessa classe di problema.
