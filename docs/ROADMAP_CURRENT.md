...
> Last updated: 2026-04-27 (B227 — Resolver `intensity_max` enforcement + 3-tier cascade. `pick_best_exercise_p0` and `_resolve_inline_block` now honor `intensity_max` from JSON `selection.primary.filters` via 7-value ordinal mapping. Catalog parity fix: `_SESSION_META["regeneration_easy"]` adds `gym_boulder`. Suite 1832 → 1843 (+11). Audit F1 closed. Opens C231 (catalog enum normalization).)
