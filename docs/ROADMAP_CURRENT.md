...
| ID | Title | Type | Effort | Status | Notes |
|----|-------|------|--------|--------|-------|
| B207 | **Harden `warmup_climbing` template — fallback for no-wall home** | B | S | Open **P2** | Residual from D210, **re-confirmed by D248 as leak L2** (sibling L3 closed by C243). `warmup_easy_boulders` referenced by explicit `exercise_id` → bypasses P0 equipment gate (`resolve_session.py:1530-1536`, inline twin `1014-1028`). Options: (a) fallback to P0 when the pinned exercise is equipment-incompatible (preferred — fixes the bypass at the root, not just this one pin), or (b) add `warmup_general_mobility` fallback. HIGH-RISK (`resolve_session`) → richiede analisi + STOP gate. Coordinare con pre-check C243 (block-empty graceful già verificato). |
