...
| A219 | **Partner Training Mode** — per-session toggle for lead/top-rope sessions. Engine restructures climbing blocks into alternating or blocked patterns where rest = partner's climb time. Mirror-partner assumption (no two-profile algo). | P3 | A | M | Open | Spec: `docs/feature_partner_mode_v1.md`. Origin: Daniele (2026-05-07). Standalone — output transformer post-resolve, no engine changes. Compatible sessions: `route_endurance_gym`, `route_projecting_gym`, `power_endurance_gym`. Recommended timing: 2-4 weeks post Stripe LIVE. Marketing angle: real differentiator vs. solo-focused competitors. |
