...
| B82 | Availability stale nel piano generato. Il piano corrente usa uno snapshot di availability che non riflette le modifiche salvate da Settings. Causa probabile: il regen del piano legge uno stato vecchio invece del user_state aggiornato su Railway. Riproducibile: utente salva Settings (es. Thu/Fri da home→gym), rigenera piano, il piano continua a usare home. Fix richiede UUID utente Railway per confrontare availability in user_state vs profile_snapshot nel piano corrente. P1. (2026-03-03) | 🔲 OPEN | engine+API | §8 |
| B83 | Session add-on modules: moduli opzionali aggiungibili a sessioni esistenti — upper_body_weights (petto/tricipiti/bicipiti, priorità alta), legs, core. Equipment-aware, load score aggiornato. Note da verifica: nessun endpoint esistente per modificare sessioni post-risoluzione; guided session usa array immutabile in localStorage; load scoring ha due sistemi separati (session_load_score = somma fatigue_cost, estimated_load_score usato da weekly_load_summary e ripple — va gestito esplicitamente); catalogo: 9 esercizi upper body OK, 3 esercizi legs insufficienti (servono lunge/goblet squat/glute bridge), core_standard template già esiste ma minimale (1 esercizio). Due approcci: (A) addon post-resolution con nuovo meccanismo iniezione, (B) mini-sessioni dedicate via quick-add esistente. | 🔲 OPEN | feature | §7 |
| B84 | Pool base insufficiente per settimane lunghe. Con 6-7 giorni di training il pass 1 piazza solo 2 sessioni climbing primary (technique_focus_gym, endurance_aerobic_gym) e i restanti giorni ricevono solo complementary. Causa: il pool base ha troppo poche sessioni primary/climbing a intensity low-medium. Fix: aggiungere route_endurance_gym e altre sessioni climbing medium intensity come primary nel pool base. Verificare tutte le fasi per copertura adeguata con availability estese. P1. (2026-03-04) | 🔲 OPEN | engine | §4 |
| B85 | Manca sessione boulder endurance per base phase. four_by_four_bouldering e emom_bouldering esistono nel catalogo ma sono intensity high → filtrati fuori dalla base phase. Serve una nuova sessione boulder_endurance_gym con intensity low/medium per circuit bouldering e linked problems a -3/-4 grade — appropriata per base phase. Arricchisce il pool climbing e contribuisce a risolvere il problema B84 per utenti con molti giorni disponibili. P2. (2026-03-04) | 🔲 OPEN | feature/catalog | §4 |
| B86 | Base phase intensity cap + pool expansion (design bug + feature). Due parti collegate. **Parte 1**: `PHASE_INTENSITY_CAP["base"]` va cambiato da `"high"` a `"medium"` — la base phase non dovrebbe ammettere sessioni high intensity per definizione. **Parte 2**: una volta applicato il cap medium, il pool base avrà poche sessioni climbing variety. Aggiungere sessioni medium intensity per boulder e corda: una sessione boulder volume/circuit (gym_boulder, medium), una sessione routes easy (gym_routes, medium, ARC-focused). Obiettivo: base phase con almeno 3-4 sessioni climbing diverse tutte medium intensity, mix boulder/corda. Supersede parzialmente B84/B85 per la base phase. P1. (2026-03-04) | 🔲 OPEN | engine+catalog | §4 |
