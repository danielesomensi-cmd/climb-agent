...
from backend.engine.cluster_utils import cluster_key_for_exercise  # noqa: E402
...
def run_resolver(tmp_path, user_state: dict, target_date: str):
    ...
    template_path = os.path.join(repo_root, "backend", "catalog", "templates", "v1", "cluster_test.json")
    session_path = os.path.join(repo_root, "backend", "catalog", "sessions", "v1", "cluster_session.json")
    ...
    return resolve_session(
        repo_root=repo_root,
        session_path="backend/catalog/sessions/v1/cluster_session.json",
