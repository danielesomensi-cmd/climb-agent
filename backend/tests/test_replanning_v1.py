...
# ── B96: Equipment-aware override ──────────────────────────────────

def _make_v2_plan(gym_equipment=None):
    ...
    domain_weights = _adjust_domain_weights(base_weights, profile)
    session_pool = _build_session_pool("base")
    equip = gym_equipment or ["gym_boulder", "gym_routes", "hangboard"]
    return generate_phase_week(
        phase_id="base",
        domain_weights=domain_weights,
        session_pool=session_pool,
        ...
        gyms=[{"gym_id": "cocque", "equipment": equip}],
    )
...
def test_b96_override_respects_gym_equipment():
    ...
    updated = apply_day_override(
        ...
        gym_id="cocque",
        gyms=[{"gym_id": "cocque", "equipment": ["gym_boulder", "hangboard"]}],
    )
...
def test_b96_override_keeps_session_when_equipment_matches():
    """B96: Endurance override at gym WITH gym_routes should still use power_endurance_gym."""
    plan = _make_v2_plan(gym_equipment=["gym_boulder", "gym_routes", "hangboard"])
    updated = apply_day_override(
        plan,
        intent="power_endurance",
        location="gym",
...
def test_b96_override_no_gyms_works_normally():
    """B96: Without gyms param, override works as before (no equipment check)."""
    plan = _make_v2_plan()
    updated = apply_day_override(
        plan,
        intent="aerobic_endurance",
        location="gym",
        reference_date="2026-01-05",
        target_date="2026-01-06",
    )
    target_day = next(d for d in updated["weeks"][0]["days"] if d["date"] == "2026-01-06")
    assert len(target_day["sessions"]) >= 1


# ── B97: Outdoor override ──────────────────────────────────────────

def test_b97_outdoor_override_sets_outdoor_fields():
    """B97: Override with outdoor intent sets outdoor fields and clears sessions."""
    plan = _make_v2_plan()
    updated = apply_day_override(
        plan,
    ...
    target_day = next(d for d in updated["weeks"][0]["days"] if d["date"] == "2026-01-06")
    assert target_day["sessions"] == [], "Indoor sessions should be cleared"
    assert target_day["outdoor_session_status"] == "planned"
    assert target_day["outdoor_discipline"] == "both"


def test_b97_outdoor_boulder_sets_discipline():
    """B97: outdoor_boulder intent sets discipline to 'boulder'."""
    plan = _make_v2_plan()
    updated = apply_day_override(
        plan,
        intent="outdoor_boulder",
        location="outdoor",
        reference_date="2026-01-05",
        target_date="2026-01-06",
    )
    target_day = next(d for d in updated["weeks"][0]["days"] if d["date"] == "2026-01-06")
    assert target_day["outdoor_discipline"] == "boulder"


def test_b97_outdoor_volume_sets_lead_discipline():
    """B97: outdoor_volume intent sets discipline to 'lead'."""
    plan = _make_v2_plan()
    updated = apply_day_override(
        plan,
    ...
    target_day = next(d for d in updated["weeks"][0]["days"] if d["date"] == "2026-01-06")
    assert target_day["outdoor_discipline"] == "lead"


def test_b97_outdoor_override_no_ripple():
    """B97: Outdoor override should not trigger ripple effect on adjacent days."""
    plan = _make_v2_plan()
    # Capture day+1 sessions before override
    day_after = next(d for d in plan["weeks"][0]["days"] if d["date"] == "2026-01-07")
    sessions_before = [s["session_id"] for s in day_after.get("sessions", [])]
