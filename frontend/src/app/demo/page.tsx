...
const FONT_DISPLAY = "var(--font-demo-display), Impact, sans-serif";
const FONT_MONO = "var(--font-demo-mono), ui-monospace, Menlo, monospace";
const FONT_BODY = "var(--font-demo-body), system-ui, sans-serif";
    ...
    <div style={{ background: C.bg, borderBottom: `1px solid ${C.rule}` }}>
      ...
      <div style={{ padding: "18px 16px 14px" }}>
        ...
        >
          ...
          <Mono size={9.5}>{DEMO_TOTAL_MIN} MIN · 08 PARTS</Mono>
        </div>
        <h1
          style={{
...
function SpecCard() {
  const rows: [string, string][] = [
    ...
    ["DURATION", `~${DEMO_TOTAL_MIN} min · 08 parts`],
...
function SectionHeader({ block }: { block: DemoBlock }) {
  ...
  return (
    <div style={{ padding: "32px 16px 20px" }} ref={endMarkerRef}>
      ...
      >
        ...
        <Mono size={9.5}>~{DEMO_TOTAL_MIN} MIN · COMPLETE</Mono>
      </div>
      <div
        style={{
          fontFamily: FONT_DISPLAY,
...
export default function DemoPage() {
  ...
  const firstHalf = DEMO_SESSION.slice(0, 4);
  const secondHalf = DEMO_SESSION.slice(4);
